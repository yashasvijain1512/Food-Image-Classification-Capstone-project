...
6) Evaluate a saved model on TFDS validation data:

```bash
python evaluate_model.py --model models/efficientnet_101_best.h5 --num-batches 20 --plot
```

7) Evaluate only selected classes using a class list file:

```bash
python evaluate_model.py --model models/efficientnet_101_best.h5 --selected-classes-file selected_classes.txt --num-batches 10 --plot --out selected_class_evaluation.json
```

8) Run the FastAPI inference server:
